...
if "SECRET_KEY_FILE" in os.environ:
    app.config["SECRET_KEY"] = (
        open(os.environ.get("SECRET_KEY_FILE")).read().strip()
    )
...
def check_auth(user):
    return user.get("username") in pwds and check_password_hash(
        pwds[user.get("username")], user.get("password")
    )
...
def upload_file():
    ...
    scheduler.add_job(
        generate_file, args=[g, g.upload_id], id=str(g.upload_id)
    )
